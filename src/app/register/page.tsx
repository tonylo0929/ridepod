...
export default function RegisterPage() {
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (result.error) {
        setError("Couldn't create account. Try again later.");
      } else {
        if (result.data.user) {
          await ensureProfileForUser(result.data.user, displayName);
        }
      ...
      <section className="mx-auto grid w-full max-w-md gap-4 rounded-[28px] border border-[var(--rp-border)] bg-[var(--rp-card)] p-5 shadow-[var(--rp-shadow-soft)]">
        ...
        <form className="grid gap-4" onSubmit={onSubmit}>
          <label className="grid gap-2">
            <span className="text-sm font-black text-[var(--rp-text)]">Display name</span>
            <input
              value={displayName}
        ...
        <p className="text-sm font-semibold text-[var(--rp-muted)]">
          ...
          <Link href="/login" className="font-black text-[var(--rp-primary)]">
            Log in
          </Link>
        </p>
      </section>
