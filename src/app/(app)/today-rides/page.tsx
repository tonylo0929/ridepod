...
function RideBoardPreviewSection({
  ...
  const isToday = selectedCategory === "today";
    ...
    >
      ...
      {requests.length > 0 ? (
        <div className="grid gap-2.5">
          {requests.map((request) => (
            <RideBoardPreviewPostCard key={request.id} request={request} accent={selectedCategory} onOpen={onOpen} />
          ))}
        </div>
      ) : (
      ...
      <Link
        ...
        className={cn(
          ...
          isToday
            ? "border-transparent bg-[#F2C15B] text-[#07131C] shadow-[0_16px_34px_rgba(242,193,91,0.24)] hover:bg-[#FFD166] focus-visible:outline-[var(--rp-primary)]"
            : "border-transparent bg-[#2563EB] text-white shadow-[0_16px_34px_rgba(37,99,235,0.24)] hover:bg-[#1D4ED8] focus-visible:outline-[#60A5FA]",
        )}
      >
        {seeMoreLabel}
...
function RideBoardPreviewPostCard({
  ...
  request: RideRequest;
  accent: RideBoardPreviewCategory;
  onOpen: (id: string) => void;
}) {
