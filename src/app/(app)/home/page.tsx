...
function getEmptyCopy(rideModeFilter: RideModeFilter, hasAnyRides: boolean) {
  if (rideModeFilter === "taxi") return "Taxi pods are coming soon.";
...
function HomeBottomActionCards() {
  return (
    <section className="mx-auto w-full max-w-[560px]" aria-labelledby="home-bottom-actions-title">
      ...
      <div className="grid grid-cols-2 gap-2.5 max-[389px]:grid-cols-1">
        <Link
          href="/today-rides"
...
function CategoryCompactResultCard({
  ...
  return (
    <Link
      href={cardHref}
      className={cn(
        "group relative grid items-center gap-3 overflow-hidden rounded-[20px] border px-3 py-3 shadow-[0_16px_34px_rgba(0,0,0,0.28)] transition min-[390px]:px-4",
        isAirportRide
    ...
    >
      ...
      <div className={cn("relative z-10 grid shrink-0 justify-items-center", isAirportRide ? "gap-1.5" : "")}>
        <span
          ...
          className={cn(
            "grid h-10 w-10 shrink-0 place-items-center rounded-full border shadow-[0_10px_22px_rgba(0,0,0,0.24)] min-[390px]:h-11 min-[390px]:w-11",
            isAirportRide
        ...
        {airportDirectionLines ? (
          <span className="grid w-full justify-items-center rounded-[10px] border border-[#ffd968]/24 bg-[#ffd968]/10 px-1 py-1 text-center text-[9px] font-black uppercase leading-[10px] text-[#ffe7a3] shadow-[inset_0_1px_0_rgba(255,255,255,0.08)]">
            <span>{airportDirectionLines[0]}</span>
            <span>{airportDirectionLines[1]}</span>
          </span>
        ) : null}
      </div>
...
function HomePageContent() {
  ...
  const tabFilteredRides = useMemo(
    ...
    [activeTab, airportDirectionFilter, airportFlightQuery, filteredRides],
  );
    ...
    >
      <section className="relative overflow-hidden px-4 pb-2 pt-7 sm:px-6 lg:px-10">
        ...
        <div className="relative z-10 mx-auto mt-5 w-full max-w-[704px] px-4 sm:px-6 lg:px-0">
          <HomeSectionHeader
            id="choose-ride-mode-title"
            title="Choose your mode"
            subtitle="Start here: choose the ride style you want to browse."
            stepLabel="1"
            icon={Smartphone}
            tone="sky"
          />
