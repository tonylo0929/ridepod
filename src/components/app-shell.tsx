  ...
  return (
    <nav className="fixed bottom-0 left-1/2 z-[120] min-h-[72px] w-full max-w-md -translate-x-1/2 border-t border-[var(--rp-border)] bg-[color-mix(in_srgb,var(--rp-shell)_96%,#02070d)] px-0 pb-[max(env(safe-area-inset-bottom),0.35rem)] shadow-[var(--rp-shadow-nav)] backdrop-blur-xl lg:hidden">
      <div className="grid grid-cols-4 items-center">
        ...
        <NavLink href="/create" label="Create your Ride" icon={PlusCircle} compact />
        <NavLink href="/pods" label="My Ride" icon={CalendarCheck} compact requiresAuth isLoggedIn={isLoggedIn} hardNavigate />
        <NavLink href="/today-rides" label="Post your Request" icon={CalendarDays} compact />
      </div>
    </nav>
  );
}
...
function LogoutSuccessToast({ message }: { message: string }) {
  return (
    <div className="fixed inset-0 z-[150] grid pointer-events-none place-items-center px-4 lg:left-72">
      <div className="mx-auto flex min-h-12 max-w-md items-center justify-center rounded-[18px] border border-[var(--rp-primary)]/45 bg-[linear-gradient(180deg,rgba(38,31,16,0.98),rgba(12,18,24,0.98))] px-4 py-3 text-center text-sm font-black leading-5 text-white shadow-[0_18px_54px_rgba(0,0,0,0.45),0_0_28px_rgba(245,197,91,0.15)]">
        {message}
      </div>
    </div>
  );
