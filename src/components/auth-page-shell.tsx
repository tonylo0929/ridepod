  ...
  return (
    <header className="sticky top-0 z-40 border-b border-[var(--rp-border)] bg-[color-mix(in_srgb,var(--rp-shell)_92%,transparent)] px-4 py-3 shadow-[0_12px_30px_rgba(0,0,0,0.14)] backdrop-blur-xl lg:hidden">
      <div className="mx-auto grid max-w-3xl grid-cols-[56px_1fr_56px] items-center gap-3">
        <HomeMenuDrawer />
        <Link href="/home" className="inline-flex items-center justify-self-center" aria-label="RidePod home">
          <RidePodLogo className="h-9 justify-center" priority />
        </Link>
        <span className="justify-self-end rounded-full border border-[var(--rp-border-strong)] bg-[var(--rp-card-soft)] px-2 py-0.5 text-[10px] font-black tracking-[0.08em] text-[var(--rp-primary)]">
          v1.0
        </span>
      </div>
    </header>
...
export function AuthPageShell({ children }: { children: React.ReactNode }) {
  return (
    <div className="premium-app min-h-screen bg-[var(--rp-gradient-app)] text-[var(--rp-text)]">
      <AuthDesktopSidebar />
      <AuthTopNav />
      <main className="mx-auto w-full max-w-3xl px-4 py-5 lg:ml-72 lg:max-w-5xl lg:px-10 lg:py-10">
        {children}
      </main>
    </div>
  );
